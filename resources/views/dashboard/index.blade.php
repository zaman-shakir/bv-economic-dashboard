        ...
        <div class="max-w-[1600px] mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex gap-1.5 flex-1">
                    ...
                    <a href="{{ route('dashboard', ['filter' => $currentFilter, 'grouping' => 'other_ref']) }}"
                       class="flex-1 px-1.5 xl:px-2.5 py-1.5 rounded-md text-sm font-medium transition text-center whitespace-nowrap flex items-center justify-center gap-1.5 {{ ($currentGrouping ?? 'employee') === 'other_ref' ? 'bg-indigo-600 text-white' : 'bg-indigo-200 dark:bg-indigo-900/50 text-indigo-800 dark:text-indigo-200 hover:bg-indigo-300 dark:hover:bg-indigo-900/70' }}">
                        ...
                        {{ __('dashboard.by_other_ref') }}
                    </a>
                </div>
                @else
                    <!-- Restricted users: show role badge instead -->
                    <div class="flex items-center gap-2 px-3 py-1.5 bg-yellow-100 dark:bg-yellow-900/30 rounded-md">
                        <svg class="w-4 h-4 text-yellow-600 dark:text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                        </svg>
                        <span class="text-sm font-medium text-yellow-800 dark:text-yellow-300">
                            Limited Access ({{ ucfirst(Auth::user()->role) }})
                        </span>
                    </div>
                @endif
                ...
                <div class="flex gap-3 xl:gap-1.5">
                    <select id="employeeFilter" onchange="filterByEmployee(this.value)"
                        ...
                        @foreach($invoicesByEmployee as $emp)
                            @php
                                // Hide "Project" and numeric reference entries when grouping by other_ref
                                $name = $emp['employeeName'];
                                $isProject = str_starts_with($name, 'Project ');
                                $isNumericRef = preg_match('/^\d+/', $name); // Starts with numbers like "108471-1"
                                $shouldHide = $isProject || $isNumericRef;
                                $shouldShow = ($currentGrouping ?? 'employee') === 'employee' || !$shouldHide;
                            @endphp
                            @if($shouldShow)
                    ...
                    @if(($usingDatabase ?? false) && Auth::user()->canSync())
                    <button
                        id="syncButton"
                        onclick="syncNow()"
                    ...
                    <div class="flex items-center gap-2 flex-wrap text-sm text-gray-600 dark:text-gray-400">
                        ...
                        @else
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-yellow-700 dark:text-yellow-400">{{ __('dashboard.sync_recommended') }}</span>
                            </div>
                        @endif
                        @if($lastSyncedAt)
                            <span>• {{ __('dashboard.last') }}: <strong>{{ $lastSyncedAt->diffForHumans() }}</strong> <span class="text-xs">({{ $lastSyncedAt->format('d M H:i') }})</span></span>
                        @else
                        ...
                        @if($nextSyncAt->isPast())
                            <span>• {{ __('dashboard.next') }}: <strong class="text-yellow-600 dark:text-yellow-400">{{ __('dashboard.overdue') }}</strong></span>
                        @else
                            <span>• {{ __('dashboard.next') }}: <strong>{{ $nextSyncAt->diffForHumans() }}</strong> <span class="text-xs">({{ $nextSyncAt->format('H:i') }})</span></span>
                        @endif
                        @endif
                    </div>
                </div>
            ...
            <div class="mb-4 card-glass p-4">
                    ...
                    <div class="flex-1 min-w-[250px]">
                        <div class="relative">
                            ...
                            <input type="text" id="searchInput" value="{{ $search ?? '' }}"
                                   placeholder="{{ __('dashboard.search_invoices') }} ({{ __('dashboard.press_enter_to_search') }})"
                ...
                <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600 flex items-center gap-2 flex-wrap">
                    <span class="text-xs text-gray-500 dark:text-gray-400 font-medium">{{ __('dashboard.quick') }}:</span>
                    @php
                    ...
                    @endphp
                    <button onclick="setDatePreset('today')" class="px-2 py-1 text-xs {{ $isToday ? 'bg-blue-600 text-white' : 'bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300' }} rounded transition">{{ __('dashboard.today') }}</button>
                    <button onclick="setDatePreset('last_3_days')" class="px-2 py-1 text-xs {{ $isLast3Days ? 'bg-blue-600 text-white' : 'bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300' }} rounded transition">{{ __('dashboard.last_3_days') }}</button>
                    <button onclick="setDatePreset('last_week')" class="px-2 py-1 text-xs {{ $isLastWeek ? 'bg-blue-600 text-white' : 'bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300' }} rounded transition">{{ __('dashboard.last_week') }}</button>
                    <button onclick="setDatePreset('this_month')" class="px-2 py-1 text-xs {{ $isThisMonth ? 'bg-blue-600 text-white' : 'bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300' }} rounded transition">{{ __('dashboard.this_month') }}</button>
                    <button onclick="setDatePreset('last_3_months')" class="px-2 py-1 text-xs bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded transition">{{ __('dashboard.last_3_months') }}</button>
                    <button onclick="setDatePreset('last_6_months')" class="px-2 py-1 text-xs bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded transition">{{ __('dashboard.last_6_months') }}</button>
                    <button onclick="setDatePreset('last_year')" class="px-2 py-1 text-xs bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded transition">{{ __('dashboard.last_year') }}</button>
                    <button onclick="setDatePreset('this_year')" class="px-2 py-1 text-xs bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded transition">{{ __('dashboard.this_year') }}</button>
                </div>
